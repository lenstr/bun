...
"git-pkg@git+https://github.com/example/repo.git#commit=abc123def456":
  version: 1.0.0
...
`,
    ...
    // Note: git-pkg doesn't resolve because package.json has #v1.0.0 but lockfile has #commit=...
    // This is a known limitation - git refs vs resolved commits don't match
...
`,
  ...
  test("patch: protocol dependencies", async () => {
    using dir = tempDir("yarn-berry-patch", {
      "package.json": JSON.stringify({
        name: "patch-test",
        version: "1.0.0",
        dependencies: {
      ...
      ".yarn/patches/lodash-npm-4.17.21-abc123.patch": `--- a/index.js
+++ b/index.js
@@ -1,1 +1,2 @@
+// patched!
...
`,
    ...
    // patch: protocol may or may not be fully supported - just check it doesn't crash
    // If it fails gracefully, that's fine for now
    if (exitCode === 0) {
      const lockfile = await Bun.file(join(String(dir), "bun.lock")).text();
      expect(lockfile).toMatchSnapshot("patch-protocol-berry");
      expect(lockfile).toContain("lodash");
    } else {
      // If migration fails for patch:, it should fail gracefully
...
`,
    ...
    // Note: __archiveUrl metadata is currently stripped during migration
    // Custom registry URL preservation would require parsing the ::__archiveUrl= suffix
  });
